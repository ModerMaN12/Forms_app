...
class _LocalHomeScreenState extends ConsumerState<LocalHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Local Surveys'),
        actions: [
          IconButton(
  ...
  Widget _buildBody(LocalSurveyState state) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('No local surveys', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.grey[500])),
            const SizedBox(height: 8),
            Text('Tap + to create a survey offline', style: TextStyle(color: Colors.grey[400])),
          ],
        ),
      );
    }
  ...
  Widget _buildSurveyCard(BuildContext context, survey) {
    return Card(
      child: InkWell(
        onTap: () => Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => LocalSurveyDetailScreen(surveyId: survey.id),
          ),
        ).then((_) => ref.read(localSurveyProvider.notifier).loadSurveys()),
              ...
              Row(
                children: [
                  ...
                  Container(
                    ...
                    child: const Text('Offline', style: TextStyle(color: Colors.blue, fontSize: 11)),
                  ),
                ],
              ),
